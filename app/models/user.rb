class User < ApplicationRecord
  ...
  def self.search(param)
    param.strip!
    to_send_back = (first_name_matches(param) + last_namme_matches(param) + email_matches(param)).uniq
    return nil unless to_send_back
    to_send_back
  end

  def self.first_namme_matches(param)
    matches('fisrt_name', param)
  end

  def self.last_namme_matches(param)
